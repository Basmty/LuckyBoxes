...
public class SkullMaker {
    ...
    public static ItemStack makeSkull(String base64Encoded) {
        if (getNMSversion().equals("1.13+")) {
            final ItemStack stack = new ItemStack(Material.valueOf("PLAYER_HEAD"));
            stack.setItemMeta(createSkullMeta(base64Encoded));
            return stack;
        } else {
            final ItemStack stack = new ItemStack(Material.valueOf("SKULL_ITEM"), 1, (short) SkullType.PLAYER.ordinal());
            stack.setItemMeta(createSkullMeta(base64Encoded));
            return stack;
        }
    }
